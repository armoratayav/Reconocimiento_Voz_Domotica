...
MODEL_PATH = MODEL_DIR / "debug_overfit_secuencial_gru_estable.keras"
...
def build_model(num_classes):
    """
    Variante secuencial mas estable:
    - GRU bidireccional para no depender solo del final del audio.
    - Pooling temporal max/average para resumir frames relevantes.
    """
...
def print_predictions(model, X, y_true, used_files, class_names):
    ...
    for file_path, true_index, predicted_index, probs in zip(
        ...
        print(
            f"{status:5s} | Real: {real_class:20s} | Pred: {predicted_class:20s} | "
...
def main():
    ...
    print()
    print("Entrenando variante estable sobre el mismo subconjunto...")
    model.fit(
        X,
        y_categorical,
        epochs=EPOCHS,
    ...
    print_predictions(model, X, y_categorical, used_files, label_encoder.classes_)
    ...
    if final_accuracy < 0.95:
        print()
        print(
            "ALERTA: la variante estable tampoco logra memorizar bien. "
            "Conviene revisar datos o probar caracteristicas Mel/CNN+GRU."
        )
